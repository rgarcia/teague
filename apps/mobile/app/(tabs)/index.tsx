...
const ApiDemoScreen = () => {
  ...
  const [showEvents, setShowEvents] = useState(() => {
    const storedLogsExpanded = storage.getString("logsExpanded");
    return storedLogsExpanded ? JSON.parse(storedLogsExpanded) : false;
  });

  // Save showEvents state to MMKV storage whenever it changes
  useEffect(() => {
    storage.set("logsExpanded", JSON.stringify(showEvents));
  }, [showEvents]);
